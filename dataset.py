...
class TrainDataset(Dataset):
    def __init__(self, paths):
        ...
        self.count={}
        for path in paths:
            self.list = os.listdir(os.path.join(path, "Image"))
            for i in self.list:
                self.image.append(os.path.join(path, "Image", i))
                self.label.append(os.path.join(path, "Scribble", i.split(".")[0] + ".png"))
...
def my_collate_fn(batch):
    ...
    for item in batch:
        ...
        strengthens.append(item['strengthen'])
